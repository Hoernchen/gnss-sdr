        ...
        unread_ (0)
{
    ...
    if (ec)
        {
            LOG(ERROR) << address << " is not an IP address" << std::endl;
    ...
    if (ec)
        {
            LOG(ERROR) << "Failed to open socket." << std::endl;
    ...
    if (ec)
        {
            LOG(ERROR) << "Failed to set reuse address option: " << ec << std::endl;
            LOG (WARNING)  << "Failed to set reuse address option";
        }
    socket_.set_option (boost::asio::socket_base::linger (true, 0), ec);
    if (ec)
        {
            LOG(ERROR) << "Failed to set linger option: " << ec << std::endl;
    ...
    LOG(ERROR) << "Connected to " << addr << ":" << port << std::endl;
    ...
    if (ec)
        {
            LOG(ERROR) << "Failed to set no delay option." << std::endl;
    ...
    if (ec)
        {
            LOG(ERROR) << "Failed to read dongle info." << std::endl;
            LOG (WARNING)  << "Failed to read dongle info";
        }
    else if (info_.is_valid ())
        {
            LOG(ERROR) << "Found " << info_.get_type_name() << " tuner."  << std::endl;
...
void rtl_tcp_signal_source_c::set_frequency (int frequency)
{
    ...
    if (ec)
        {
            LOG(ERROR) << "Failed to set frequency" << std::endl;
...
void rtl_tcp_signal_source_c::set_sample_rate (int sample_rate)
{
    ...
    if (ec)
        {
            LOG(ERROR) << "Failed to set sample rate" << std::endl;
...
void rtl_tcp_signal_source_c::set_agc_mode (bool agc)
{
    ...
    if (ec)
        {
            LOG(ERROR) << "Failed to set gain mode" << std::endl;
...
void rtl_tcp_signal_source_c::set_gain (int gain)
{
    ...
    if (ec)
        {
            LOG(ERROR) << "Failed to set gain" << std::endl;
    ...
    for (unsigned stage = 1; stage <= gains.size(); stage++)
        {
            ...
            if (ec)
                {
                    LOG(ERROR) << "Failed to set if gain" << std::endl;
        ...
        size_t bytes_transferred)
{
    if (ec)
        {
            LOG(ERROR) << "Error during read: " << ec << std::endl;
