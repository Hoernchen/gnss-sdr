...
bool FrontEndCal::read_assistance_from_XML()
{
    ...
    LOG(ERROR) << "SUPL: Trying to read GPS ephemeris from XML file " << eph_xml_filename << std::endl;
    ...
    if (supl_client_ephemeris_.load_ephemeris_xml(eph_xml_filename) == true)
        {
            ...
            for(gps_eph_iter = supl_client_ephemeris_.gps_ephemeris_map.begin();
                    ...
                    gps_eph_iter++)
                {
                    LOG(ERROR) << "SUPL: Read XML Ephemeris for GPS SV " << gps_eph_iter->first << std::endl;
        ...
        {
            LOG(ERROR) << "ERROR: SUPL client error reading XML" << std::endl;
        ...
        //SUPL SERVER TEST. Not operational yet!
        {
            ...
            if (SUPL_read_gps_assistance_xml == true)
            ...
            else
                {
                    ...
                    LOG(ERROR) << "SUPL: Trying to read GPS ephemeris from SUPL server..." << std::endl;
                    ...
                    if (error == 0)
                        {
                            ...
                            for(gps_eph_iter = supl_client_ephemeris_.gps_ephemeris_map.begin();
                                    ...
                                    gps_eph_iter++)
                                {
                                    ...
                                    LOG(ERROR) << "SUPL: Received Ephemeris for GPS SV " << gps_eph_iter->first << std::endl;
                                    LOG(INFO)  << "New Ephemeris record inserted with Toe=" << gps_eph_iter->second.d_Toe << " and GPS Week=" << gps_eph_iter->second.i_GPS_week;
                                    global_gps_ephemeris_map.write(gps_eph_iter->second.i_satellite_PRN, gps_eph_iter->second);
                                }
                    ...
                    else
                        {
                            ...
                            LOG(ERROR) << "ERROR in SUPL client. Please check your Internet connection and SUPL server configuration" << std::endl;
                        }
                    ...
                    LOG(ERROR) << "SUPL: Trying to read Acquisition assistance from SUPL server..." << std::endl;
                    ...
                    if (error == 0)
                        {
                            ...
                            for(gps_acq_iter = supl_client_acquisition_.gps_acq_map.begin();
                                    ...
                                    gps_acq_iter++)
                                {
                                    ...
                                    LOG(ERROR) << "SUPL: Received Acquisition assistance for GPS SV " << gps_acq_iter->first << std::endl;
...
bool FrontEndCal::get_ephemeris()
{
    ...
    if (read_ephemeris_from_xml == true)
        {
            LOG(ERROR) <<  "Trying to read ephemeris from XML file..." << std::endl;
            LOG(INFO) << "Trying to read ephemeris from XML file...";
            if (read_assistance_from_XML() == false)
                {
                    LOG(ERROR) << "ERROR: Could not read Ephemeris file: Trying to get ephemeris from SUPL server..." << std::endl;
        ...
        {
            LOG(ERROR) << "Trying to read ephemeris from SUPL server..." << std::endl;
